/* ===== Back2Maps — bubbles on top + regional choropleth + smooth zoom fade ===== */
(() => {
  "use strict";

  // Safe-guard missing jQuery UI progressbar (theme quirk)
  if (window.jQuery && !jQuery.fn.progressbar) jQuery.fn.progressbar = function(){ return this; };

  // Inject a tiny CSS transition so polygon opacity changes fade smoothly
  (function injectCSS(){
    const css = `
      #b2m-map .leaflet-interactive {
  ...
  // keep references so we can update tooltips + styles live
  ...
  // smooth fade between states (zoomed-out) and regions (zoomed-in)
  function applyZoomFade(map){
    const z = map.getZoom();
    const showRegions = z >= DIV_ZOOM;
    if (statesLayer) {
      statesLayer.eachLayer(l=>{
        l.setStyle({ fillOpacity: showRegions ? 0.18 : 1, opacity: 1 });
      });
    }
    if (regionLayer) {
      regionLayer.eachLayer(l=>{
        l.setStyle({ fillOpacity: showRegions ? ( (DIV_COUNTS.size?0.65:0.20) ) : 0, opacity: showRegions ? 1 : 0 });
      });
    }
    if (bubbleLayer) bubbleLayer.bringToFront(); // keep bubbles above
  }
  ...
  async function buildMap(){
    ...
    if (statesFC?.features){
      statesLayer = L.geoJSON(statesFC, { style: styleStates }).addTo(map);
    }
    if (regionsFC?.features?.length){
      regionLayer = L.geoJSON(regionsFC, {
        style: styleRegions,
        onEachFeature: (feat, layer) => {
          const id = feat.properties._b2m_id;
          regionLayersById.set(id, layer);
          // initial tooltip (count 0)
          const title = `${regionName(feat.properties)}<br><small>0 report(s)</small>`;
          layer.bindTooltip(title, { sticky:true });
        }
      }).addTo(map);
    }

    // Bubbles layer (on top, no popups/tooltips)
    ...
    // Plot bubbles (NO tooltip/popup bound for privacy)
    let plotted = 0;
    for (const r of rows){
    ...
    // Apply choropleth colours + region tooltips
    ...
    // Fade once now, and on every zoom
    applyZoomFade(map);
